...
- **Two custom items that draw their own glass no longer overlap mid-morph:** A matched pair of `GlassBarItemBackground.own` items — one route's capsule replacing another's under the same id — cross-faded like plain content, so for the middle of the transition both surfaces were drawn on top of each other, each sampling the other, and the overlap read as a brighter pad inside the incoming capsule that dropped out when the fade ended. They now take turns on the windows a cluster only one route has already uses: the outgoing surface dissolves, then the incoming one materializes.
