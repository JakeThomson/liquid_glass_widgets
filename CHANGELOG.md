...
- **Native capsule morph for the pinned navigation bar:** The trailing actions capsule now reshapes the way the iOS 26 bar does instead of gliding between widths. The cluster's width and item positions ride the package's bouncy spring profile for the full length of the route transition, settling in the same breath the page lands, while a gel pulse inflates the whole pill — height, radius and glyphs together, as real geometry through the glass renderer — and the spring's landing overshoot squeezes it back. Glyphs no longer plainly cross-fade: an outgoing glyph smears away under heavy blur while the capsule reshapes beneath it, and an incoming one arrives soft and sharpens last. A pop plays the same forward choreography toward the other cluster rather than the push in reverse, and a cluster only one side has gels in and out through scale instead of switching at the transition midpoint — scale being the one dimension glass can safely animate. Fully driven by the route clock, so an interactive back-swipe scrubs the whole choreography. Timing and blur constants live together in `GlassNavPinnedMetrics`; the choreography is locked down by a frame-by-frame trace test.
