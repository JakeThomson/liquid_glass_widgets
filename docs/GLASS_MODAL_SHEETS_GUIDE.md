...
**How a swipe behaves.** Below the lowest detent the sheet follows the finger on both axes and shrinks with the vertical travel:

- **The whole curve is normalised by the card's height, not the screen's.** Measured off iOS 26 by tracking the cursor against the card through iPhone Mirroring: a small panel shrinks faster per pixel of drag than a tall sheet (Maps' Map Modes panel measures ~2.5x per screen height, a medium sheet ~1.4x — one gain of 0.64 per card height explains both).
- **The shrink pivots at the grabbed point**, carried down with the fall — so up to the damping knee the content under your finger stays exactly under your finger, which is the feel that makes it direct manipulation rather than an animation. A gain below 1.0 per card height is also what keeps the card's underside off screen: the fall always outruns the shrink, so the bottom edge never lifts into view, whatever you grabbed.
- **The fall eases.** The card tracks the finger 1:1 to 0.48 of its own height, then rubber-bands — it keeps giving a little, but a long drag parks it near the bottom instead of posting it off the screen while the finger is still moving, and the anchor releases there (native does the same: the grab stays pinned "until the very last point"). The sheet's own position keeps tracking 1:1 underneath, so the dismiss threshold is unaffected: the card can be visually parked and a release still commits.
- **Scale is strictly linear in that damped travel**, easing toward a floor of 0.33 it never quite reaches (the reference gesture parked at ~0.55 and was still creeping). Size and position ease off the same curve and settle as one object. Horizontal movement never feeds the scale.
- **The sideways axis stays shut** until the downward drag is genuinely under way — `kTouchSlop` worth of travel past the detent, the same bar the gesture system uses for "a drag has started". Before that a sideways wobble belongs to the sheet's own jelly-follow stretch and nothing else. It opens anchored at the crossing, so wander from before does not snap in.
- **Once open, the card chases the finger sideways through a stiff, critically damped tracking spring** rather than copying its x — which is where the sideways weight comes from. Tracked against a native capture: a fast sweep trails the finger by ~10–15 % and catches up (briefly even overtaking) the moment the finger slows, while a slow drag reads as 1:1 — a signature no static response curve can produce, and exactly what a chase spring does for free. The spring's target is positional: free while the card has room, then pinned when its leading edge meets the screen edge, with only a few points of give (largest native overshoot measured ~4 pt). Resistance follows where the card *is*, not how far the finger has moved — grab a card against the left edge and you can push it most of the way across before it stiffens, but pull it the other way and it resists at once.
